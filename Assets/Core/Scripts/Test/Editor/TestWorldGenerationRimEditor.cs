...
[CustomEditor(typeof(TestWorldGenerationRim))]
public class TestWorldGenerationRimEditor : Editor
{
    SerializedProperty BlockParent;
    SerializedProperty BlockOffset;
    SerializedProperty BlockOffsetY;
    SerializedProperty ChunkSize;
    SerializedProperty SubChunkSize;
    SerializedProperty LayerNames;
    ...
    void OnEnable()
    {
        ...
        BlockOffsetY = serializedObject.FindProperty("BlockOffsetY");
        ChunkSize = serializedObject.FindProperty("ChunkSize");
        SubChunkSize = serializedObject.FindProperty("SubChunkSize");
    ...
    public override void OnInspectorGUI()
    {
        ...
        //Generation options
        int maxChunkSize = Mathf.CeilToInt((float)IslandXZ.intValue / (float)ChunkSize.intValue);
        EditorGUILayout.LabelField("Chunk Dimensions: " + maxChunkSize + "x" + maxChunkSize);
        EditorGUILayout.LabelField("Number of Chunks: " + maxChunkSize * maxChunkSize);
        EditorGUILayout.PropertyField(BlockParent, true);
        EditorGUILayout.PropertyField(BlockOffset, true);
        EditorGUILayout.PropertyField(BlockOffsetY, true);
        EditorGUILayout.PropertyField(ChunkSize, true);
        EditorGUILayout.PropertyField(SubChunkSize, true);
        EditorGUILayout.PropertyField(LayerNames, true);
            ...
            for (int y = 0; y < MapXZSize; y++)
            {
                ...
                switch (PreviewGenerationMask.enumValueIndex)
                {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        textureCoord[x, y] = GradientNoise(x, y, MapXZSize, MaskRadius.floatValue, textureCoord[x, y], 2);
                        break;
                }
